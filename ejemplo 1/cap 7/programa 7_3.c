...
Observa que esta función baja automáticamente una linea después de imprimir
la cadena. */
 printf("\nLa cadena cad1 es: ");
printf("%s", cad1); 
/* La función printf, con el formato de variable %s, también se puede utilizar
para escribir cadenas de caracteres. Baja automáticamente una linea después
...
queremos ingresar la cadena “Buenos dias", sólo lee la palabra “Buenos", por
...
que puedas observar la relación directa con las lineas de programación que se
muestran a continuación. */
printf("\nIngrese una linea de texto —se lee cada caracter con getchar—: \n");
/* Se utiliza la función getchar para leer caracteres de la linea de texto y
